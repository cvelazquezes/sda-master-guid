...
class SecureStorage {
  ...
  async saveCsrfToken(csrfToken: string): Promise<void> {
    ...
    try {
      ...
      logger.error('Failed to save CSRF token', error as Error);
      throw new SecureStorageError('Failed to save CSRF token');
    }
  }
  ...
  async getCsrfToken(): Promise<string | null> {
    ...
    try {
      ...
      logger.error('Failed to retrieve CSRF token', error as Error);
      return null;
    }
  }
